...
export default function Architecture() {
    ...
    return (
      ...
      `}</style>
            ...
            {/* Floating Tech Cards (Large screens only) */}
            <div className="absolute inset-0 pointer-events-none hidden xl:block z-0 overflow-hidden max-w-[1400px] mx-auto">
                {/* Left Side */}
                <div className="absolute top-[20%] left-[5%] w-32 h-40 bg-[rgba(5,11,24,0.4)] border border-electric-cyan/20 backdrop-blur-md rounded-xl flex flex-col p-4 transform -rotate-12 animate-float-slow shadow-[0_0_30px_rgba(0,242,255,0.05)]">
                ...
                <div className="absolute top-[60%] left-[8%] w-28 h-24 bg-[rgba(5,11,24,0.4)] border border-fuchsia-500/20 backdrop-blur-md rounded-xl flex flex-col p-4 transform rotate-6 animate-float-delayed shadow-[0_0_30px_rgba(217,70,239,0.05)]">
                ...
                {/* Right Side */}
                <div className="absolute top-[35%] right-[5%] w-32 h-28 bg-[rgba(5,11,24,0.4)] border border-soft-violet/20 backdrop-blur-md rounded-xl p-4 transform rotate-6 animate-float-delayed shadow-[0_0_30px_rgba(168,85,247,0.05)]">
                ...
                <div className="absolute bottom-[20%] right-[8%] w-28 h-32 bg-[rgba(5,11,24,0.4)] border border-white/10 backdrop-blur-md rounded-xl p-4 transform -rotate-12 animate-float-slow shadow-[0_0_30px_rgba(255,255,255,0.05)]">
            ...
            <main className="relative px-6 py-8 max-w-md mx-auto z-10 w-full">
                ...
                <div className="relative min-h-[720px] w-full">
                    <svg className="connector-svg" preserveAspectRatio="none" viewBox="0 0 100 720">
                        <path className="path-glow" d="M 25 30 C 25 80, 75 80, 75 140"></path>
                        <path className="path-glow" d="M 75 140 C 75 200, 25 200, 25 260"></path>
                        <path className="path-glow" d="M 25 260 C 25 320, 75 320, 75 380"></path>
                        <path className="path-glow" d="M 75 380 C 75 440, 25 440, 25 500"></path>
                        <path className="path-glow" d="M 25 500 C 25 560, 75 560, 75 620"></path>
                    </svg>

                    {nodes.map((node) => (
                        <div key={node.id} className={`relative flex flex-col w-full mb-14 ${node.align === 'left' ? 'items-start' : 'items-end'} ${activeNode === node.id ? 'z-50' : 'z-20'}`}>
                            <div className={`flex flex-col ${node.align === 'left' ? 'items-start' : 'items-center'} w-full`}>
                                <button
                                    onClick={() => setActiveNode(activeNode === node.id ? null : node.id)}
                                    className={`circular-glass-node size-14 mb-2 cursor-pointer transition-transform hover:scale-110 ${node.align === 'left' ? 'ml-[calc(25%-28px)]' : 'ml-[calc(50%)]'} ${activeNode === node.id ? 'active-node scale-110' : ''}`}>
                                    <span className="material-symbols-outlined text-primary text-xl">{node.icon}</span>
                                </button>

                                {/* Popup Detail Card */}
                                {activeNode === node.id && (
                                    <div className={`absolute ${node.align === 'left' ? 'left-8 top-16' : 'right-8 top-16'} z-50 w-[280px] matte-card p-4 rounded-xl border-primary/50 shadow-2xl shadow-primary/20`}>
                ...
                <div className="mt-20 pb-16 relative z-10">
                    <h3 className="text-base font-bold mb-6 flex items-center gap-3">
                        <span className="material-symbols-outlined text-primary text-xl">analytics</span>
                        System Specs
                    </h3>
                    <div className="grid gap-4 w-full">
                        <div className="matte-card p-5 rounded-xl flex items-center gap-4 border border-primary/20">
